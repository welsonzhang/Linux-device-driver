...
static int __init mempool_cache_init()
{
  ...
  printk(KERN_ALERT "mempool cache init.\n");
  return 0;
}

static void __exit mempool_cache_exit()
{
  if(datap)
  {
  ...
  printk(KERN_ALERT "mempool cache exit.\n");
}

MODULE_LICENSE("GPL");
module_init(mempool_cache_init);
module_exit(mempool_cache_exit);
